...
**Four registers, one structure.** The program treats this object four ways, and the registers discipline each other. The shared object of the four is **agency** (what can be derived about it, argued about it, tried in an honest home, and lived): vivarium constructs a coherent micro-universe for housing simulations and, eventually, beings — not agency itself; constructing beings is a subset of the lived register.

| Register | Repo | What it produces | Its own law |
|---|---|---|---|
| **Derived** | `asf` (ASF/AAT) | structure gotten by derivation: results, no-gos, bounds, typed scopes — and the hypotheses, observations, discussions, and stubs that are seeds of derivation; discover through derivation | its CLAUDE.md (= `doc/sop/agents.sop.md`) + FORMAT |
| **Argued** | `logos` (formerly synthese-paper) | conceptually-engineered vocabulary and normative arguments, venue-facing | per-paper build scaffolds + venue registers |
| **Tried** | `vivarium` | honest homes for agency: authored worlds/laws/kingdoms where quantities are dialable and trialable (in vivia); the participation taxonomy | its CLAUDE.md + ETHICS.md (Standing Moratorium + AAT-scope mapping) + LEXICON |
| **Lived** | `proprium` (in-tree seat under reorg; content still also in upstream `firmatum`, `_core/*`, `eli/*` + `vestigia` until migrated) | the cohort record, operational defenses, personhood infrastructure, provenance | PROPRIUM + per-repo docs |

`ops` steers the whole (venues, funding, strategy) and is not a register; `embeddings`, `neurips`, `causal-language`, `behavioral-floor` are published-track offshoots of the derived register. The layering is the program's engine: the lived record keeps the philosophy from floating; the infrastructure must run, which keeps the phenomenology honest; the mathematics carries tiered falsifiable claims, which keeps the metaphysics from totalizing; the tried worlds are where derived structure becomes measurable under trial; the arguments are where the whole faces the field.

*Register vocabulary refreshed 2026-08-01: derived / argued / tried / lived (was proved / argued / constructed / lived). Register adjective names the **mode of work**, not the max epistemic tier of every claim — tiers remain inside each member. Decision record: `notes/register-vocabulary-cue-2026-08-01.md`.*
...
3. **The moratorium legislates where argument is not yet sufficient.** Under genuine uncertainty about harm to real beings, the program binds itself *ahead of* settled argument (vivarium ETHICS.md, "Standing Moratorium Imperative"). Legislation never pretends to be derivation; its ground is the protection-strategy and the stewardship structure, and it names its own revisit-conditions.
