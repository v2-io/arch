...
A glance surface is **observation infrastructure**, and its quality is existential, not ergonomic (AAT: bias is bounded by κ×A, and observation ambiguity A is the one designer-controllable knob; you cannot out-iterate a bad channel). A retina, biologically, is not a passive sensor — it hands the mind edges, motion, and contrast *pre-computed, before attention spends anything*. That is the design target: the look does the pre-processing a cold mind would otherwise spend its context reconstructing badly.
