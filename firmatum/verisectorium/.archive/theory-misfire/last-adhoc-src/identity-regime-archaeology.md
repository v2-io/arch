...
One claim can be followed across all three. What was `FP-002` became `T-02 — Implementation Time Lower Bound` and is now `result-specification-bound`: the same claim, that the time to implement a feature is bounded below by the time to specify it, generalized on the way (the current statement is an information-transmission bound of which written specification is a special case) and re-typed on the way too (regime 2 typed it `Theorem`; regime 3 types it `result`, in a vocabulary that deliberately avoids theorem-language). So the claim did develop — but its *name* changed twice for reasons that had nothing to do with the development, and under regimes 1 and 2 a name change meant rewriting the filename, the frontmatter, the document heading, and every citation, because the identity was carried in all four at once.
...
Counted in `~/src/arch/asf/02-tst-core/src/` on 2026-08-06: total files; files not matching `old-tst-*`; and `grep` counts of the `label:` and `older-tag:` frontmatter keys across the `old-tst-*` set. The three-way correspondence for one claim was read directly from `old-tst-t02-implementation-time-lower-bound.md` (frontmatter `label: T-02`, `older-tag: FP-002`, title *Implementation Time Lower Bound*) and `result-specification-bound.md`. The exemption rule was read in `~/src/arch/asf/FORMAT.md` (§Segment files / cadence).
