...
*Convention is inherited by reference, never restated.* **comproprium**'s format document is titled *FORMAT — divergences only* and opens by declaring itself so: *"The conventions here are **ported by reference**, not restated… Restating it here would fork it — an inlined definition is not a copy, and there is nothing holding two copies in step"* — then names what is carried unchanged, so nobody has to diff. Its own inheritance is a three-link chain, stated in that opening: comproprium ports from the udon theory FORMAT, which ports from ASF's. **vivarium** states the same rule from the failure side, in `FORMAT.md` §"Do not restate a definition that lives elsewhere": *"**An inlined definition is not a copy. It is a fork.**… This is not hypothetical: `ETHICS.md` restated the LEXICON by hand and thereby used a name the dictionary had retired four days earlier, invented two access channels that do not exist, and dropped two that do."*

*And the format rules bind every file, not only the atoms.* vivarium splits its format law explicitly — §§1–4 govern claim segments; §§5–6 (cross-references, prose voice, math) *"govern every file in the repository, segment or not: … A spike, a memo, a README and a design doc are bound by them exactly as a segment is"* — with the exemption named and refused: *"'It is only a working document' is not an exemption, and the one time that exemption was taken, the result was a document that used a term the dictionary had retired four days earlier."* This is not pedantry about memos. It is the recognition that a corpus's references and vocabulary are corpus-wide properties, so a surface exempted from them becomes the place where drift enters and is not caught.

Both rules describe the shape a **generator** would have to emit — a per-instance convention file that is a declared delta over a named parent, never a copy of it — which is why they matter to ch. 11 as much as here ([[derivation-not-templates]]).
...
The inheritance-by-reference rule is stronger: stated in two corpora and re-verified verbatim first-hand this pass against the live trees (`~/src/arch/proprium/comproprium/FORMAT.md` head; `~/src/arch/vivarium/FORMAT.md` §5 and §"Do not restate a definition that lives elsewhere"), with a recorded failure behind the second — a hand-restated definition going stale in four days. The everything-binds rule is vivarium's stated law with **the same incident** as its warrant, so the two rules share a specimen and must not be counted as two independent supports.
