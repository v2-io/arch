...
- **2026-08-05 dispatch (fresh-eyes pass).** The udon-theory copies named above have been archived — cite the live corpus `~/src/arch/firmatum/udon/v2/theory/src/`. A **second specimen** for the asymmetry, from the failure side, is registered as R20 in `plan/TODO.md`: append-only supersession could not signal deadness in autopax (a body banner was needed because frontmatter supersession did not stop readers) — differently sited from the vivarium rename case, and the register also carries the reading that `empirica:`, `OUTLINE-accepted.md` and `lint-outline`'s stage check are one asymmetry seen from several sides.
