...
- **2026-08-05 dispatch (fresh-eyes pass).** The gather inventory has moved to `plan/.integrated/`; the live-state health reports remain in INFLUX. Residue bearing on this segment is registered in `plan/TODO.md`: R13 (the two candidate outside-authorship legs, which is exactly this segment's stated raise-condition) and R11 (relata is the fourth generation of a documented lineage, and the most-evolved instance is the one whose store left the code repository).
