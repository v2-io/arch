...
| Hypothesis | [[claim-session-cycle-fit]] | Atomized work grants agents concrete start/work/finish cycles without the completion ethos infecting the living whole — the *proposed* mechanism behind the pattern's success under ad-hoc application; observed, plausible, unmeasured | empirical | pre-drafted |
...
| Formulation | [[form-addressing]] | Layout-independent addressing: slug + section + word-anchor spans for provenance, quizzes, and cross-refs alike; designator resolution as a ladder (auto → interactive → batch → pending → spool), never silent best-guess (orientation-gate + comproprium check + relata ladder, unified) | decided | pre-drafted |
...
| Derived | [[claim-absence-vs-conflict]] | Absence (bare gap, unrun check) never reads as refutation or success; conflict (present-truth collision, support collision) must surface and block premature "done" | robust-qualitative | pre-drafted |
...
| Formulation | [[form-standing-verification]] | Three verification tempos: continuous lint (shallow), standing support-audit (does the cited atom still assert what dependents assumed), episodic de-novo (deep); the middle tier is the estate's missing organ-part | decided | pre-drafted |
...
| Formulation | [[form-decision-records]] | Dispositions and adjudications leave append-only events with reasons, revisit-when, expires-on — one schema family from working-note item to governance decision | decided | pre-drafted |
...
| Discussion | [[disc-epistemic-merge]] | Cross-instance and cross-mind flux: trust-modulated subgraph exchange with sandboxed integration and rollback; congruence as the persuasion criterion; the knowledge-commons horizon (ennaos EMR — far-future, principled) | discussion-grade | pre-drafted |
...
| Formulation | [[form-generated-views]] | Generated views (lexicon, findings, digests, emitted bibliographies) are derived artifacts declared as such — clobber-guarded, never hand-edited; derived-vs-authored is a declaration, not a naming convention | decided | pre-drafted |
...
| Formulation | [[form-pedagogical-ordering]] | Ordering and framing prose are view properties; mental-model-first preambles; narrative outlines with non-authoritative connective prose as a legitimate view kind (the-chain counter-case) | decided | pre-drafted |
...
| Formulation | [[form-front-door]] | The front-door layer: what loads always (disposition, index) vs on-demand (procedure), de-novo priming discipline (auditor-safe vs priming-heavy surfaces), and the currently-scattered carriers gathered into one designed organ | decided | pre-drafted |
...
| Formulation | [[form-evidence-gather]] | For interpolation-hazardous segments, drafting is preceded by a parallel estate-wide evidence gather (per-aspect sweep agents, verbatim extracts with provenance, oldest-wins, whole-file references) read whole before composing — first run 2026-08-09 (the disposition family), method feedback in the sop influx | decided | pre-drafted |
...
- **`pre-drafted` state (2026-08-09):** a stub exists in `src/` at full cadence-conformance (frontmatter + title + summary) but carrying **Working Notes only** — cautions, source pointers, and coord context that would otherwise be lost at turnover. It is a forward state between `proposed` and `drafted`: the population's tooling may rely on the file existing; readers must not cite it as a drafted segment. Introduced to distribute the misfire drafting-cautions block to its atoms and to bank the 2026-08-09 session's read-context.
...
- **Provisional gaps + drafting cautions from misfire feedback (2026-08-06; integrated from `steward-brainstorms/feedback-from-misfire.md` → `.integrated/`, salt applied):** the per-row drafting cautions formerly carried here as one block have been **distributed to their segments' Working Notes** (2026-08-09) — each named row now exists at least as a `pre-drafted` stub carrying its own caution, so the caution travels with the atom instead of living in outline prose. Items (H) dependency re-pricing and (I) graduation historical-completeness landed at [[form-standing-verification]] and [[def-integration-replacement]] respectively; the pedagogy caution (bridge/Feynman-brief segment kinds may never appear unless named) has no row yet and stays here as the block's one residue.
