...
| Formulation | [[form-pending-surface]] | Unfinished work is a queryable surface over events and gaps (with exact next command), not scattered lists (relata `pending` transfer) | decided | proposed |
...
| Formulation | [[form-view-filters]] | Views filter within atoms (by section, state-flag, tag); working notes stay in atoms and are filtered from promoted views — evergreen atoms, audience layers at the view level (§H.5) | decided | proposed |
