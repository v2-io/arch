...
- **Segments:** one atom per file in `src/`, slug = filename, form-kind prefix, the exemplars' cadence (frontmatter → title → summary → Formal Expression → Epistemic Status → Discussion → Working Notes). Frontmatter schema is provisional pending the epistemology decision — say so in each segment's Working Notes. Statuses conservative: do not borrow rigor from source confidence ([[REGISTER-RULING]]-class caution generalizes: same-author estate agreement is coherence, not corroboration). `md-press --check` every touched `.md` before reporting it done; md-press will not run on udon files unless it is forced to (no udon linter/canonicalizer exists quite yet), so hand-edit `.un` files with corresponding care.
