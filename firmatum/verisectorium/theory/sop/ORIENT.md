...
This is the founding seed of this instance's **SOP store** ( [[form-sop-realization]], [[form-store-triplet]]) — deliberately minimal: one file, three orientations, its own feedback channel. It exists so that the work continues with fidelity after any session ends. It will grow into a proper store (outline + sop segments) as its content earns atomization; until then this file is the store.
...
- **Segments:** one atom per file in `src/`, slug = filename, form-kind prefix, the exemplars' cadence (frontmatter → title → summary → Formal Expression → Epistemic Status → Discussion → Working Notes). Frontmatter schema is provisional pending the epistemology decision — say so in each segment's Working Notes. Statuses conservative: do not borrow rigor from source confidence ([[REGISTER-RULING]]-class caution generalizes: same-author estate agreement is coherence, not corroboration). `md-press --check` every touched `.md` before reporting it done; md-press will not run on udon files unless it is forced to (no udon linter/canonicalizer exists quite yet), so hand-edit udon files with corresponding care. Udon suffixes: `.ud` default; `.un` and `.us` (udon store) allowed, as are `.udon` and `.don` (bowl — provisional).
