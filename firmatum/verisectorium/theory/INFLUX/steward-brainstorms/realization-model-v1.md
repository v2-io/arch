...
history  (changelog)  the instance's record-grain history layer — REQUIRED
                     from birth [J diagnosis, 2026-08-07]: without it the
                     record-instinct (O16) carves what-happened into headers,
                     preambles, and working notes — the paths instance's
                     founding session demonstrated this live, with the coord
                     doing the carving despite knowing the rule. A CHANGELOG
                     seed belongs in the essential template
