<!--
  ...
  Provenance: asf/doc/sop/audit.sop/routing.sop.md
  Copied: 2026-08-06 (Joseph-directed; supersedes the 00-INDEX named absence for the SOPs)
  Source path at copy time: /Users/josephwecker-v2/src/arch/asf/doc/sop/audit.sop/routing.sop.md
...
**The gold lift (incidental-gold routing)** *(current ops — the influx-side companion to the gold standing gate above)*: de-novo audits produce a *second* output alongside certified findings — *incidental gold* in the per-segment reflections / §14 Wandering Thoughts (pedagogical framing, analogies, candidate figures, naming, forward-vision, aspirational reach; see `de-novo.sop.md` §7.15). It is routed **separately from findings**, per-segment, into the segment's `## Working Notes` in six categories (candidate Brief prose / candidate Discussion / follow-up items / readers-often-ask / candidate figures / belongs-elsewhere), deduplicated across substrates and lightly attributed. This is a **standing, prompt step, not a backlog** — lift each audit's gold soon after it lands so it does not pool (the pooling is the debt this step exists to prevent). Disciplines: **match by segment content, never by note-number** (OUTLINE numbering and slugs drift across audits — the same segment has been filed under wildly different note-numbers); a *batched* reflection (some runs reflect every Nth segment) carries gold for several segments — route each piece to the segment it concerns, and leave its source file in place until *every* segment it covers is swept, then move it whole; **preserve the early finding-vs-framing conflation** as signal rather than sanitizing it. Filing: as each segment's gold is lifted, its dedicated source-note moves into the per-dir `audits/AUDIT-WORKING-<id>/.integrated/` (so each dir's top level shows only still-to-sweep notes and graduates once empty) — move ONLY the exact segment; adjacent same-family segments are distinct. Promotion of staged Working-Notes gold into a segment's Brief / Discussion is a separate, careful later pass paced with the pedagogy work, not part of the lift. The gold standing gate above is **satisfied for this agreed lift** (Joseph-directed 2026-05-30) and still governs any processing beyond it.
