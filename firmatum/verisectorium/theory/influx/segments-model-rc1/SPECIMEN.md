...
- *shown-independent agreement*: **shown** — derivation + in-vivo agree and their descent is independent (different method families, no shared seed). Carriage did **not** contribute and could not have alone: had its source been another *derivation*, the descent check would look through the carriage wrapper, see same family, and give the agreement ~zero weight — carried material never corroborates a line it could have seeded.

**As of e8 (the era bumped):** `evidence/in-vivo` shows *expired @ k1 — re-run pending*; shown-independent agreement **lapses** (one counting line left); nothing was edited anywhere — the same trail simply computes differently, and the minted question routes the re-run. When e9 lands a fresh committed run under k2, the agreement is shown again; that event's criterion is the *same pre-committed one from e3a*, which is what makes the re-run certifying rather than tuning.
