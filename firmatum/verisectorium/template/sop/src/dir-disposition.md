...
- **File hygiene:** `md-press --check` every touched `.md` before reporting it done. Udon files: `.ud` is the default suffix; `.un` and `.us` (udon store) are also allowed, as are `.udon` and `.don` (bowl — provisional, until nailed down). md-press will not run on udon files unless it is forced to (no udon linter/canonicalizer exists quite yet).
