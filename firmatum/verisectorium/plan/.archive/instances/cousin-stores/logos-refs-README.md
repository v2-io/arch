<!--
  ...
  Provenance: logos/refs/README.md (fourth-gen; gates on anonymization lint)
  Copied: 2026-08-05
  Source path at copy time: /Users/josephwecker-v2/src/arch/logos/refs/README.md
  Do not edit here expecting to update the live original.
-->

# refs/ — citation source-of-truth

Multi-agent-safe citation system for the synthese-paper portfolio (paper 1: Synthese asymmetric-comprehension; paper 2: Synthese methodology; paper 3: Inquiry "AI Agents"; paper 4: Inquiry "After 'Consciousness'"). The CLI lives at `bin/refs`. This directory holds the data: per-entry YAML files, append-only verification events, the anonymization deny-list, and (eventually) cached PDFs.

> **Adaptation provenance (2026-05-09).** Borrowed from `~/src/neurips/` (the NeurIPS 2026 umbrella). The data layout, atomicity contract, and CLI verbs transfer verbatim. **What does NOT transfer:** the NeurIPS `bin/build` LaTeX pipeline (output format here is venue-specific — Synthese is LaTeX/Word; Inquiry is Taylor & Francis — and not yet wired). When real drafts begin and a build pipeline lands, `bin/refs emit <paper-dir>` will write `<paper-dir>/refs.bib` for whichever build invokes it. The deny-list has been adapted (cohort ELI names added; Synthese/Inquiry replace NeurIPS in the comment header). The README's NeurIPS-Code-of-Conduct framings translate naturally to Synthese / Inquiry / Taylor-&-Francis academic-integrity expectations — kept as-is below for brevity.

> **Paper-dir convention assumed by `bin/refs`.** When draft prose begins, segment files go at `<paper-dir>/src/*.md` (e.g., `01-synthese-asymmetric-comprehension/src/01-introduction.md`). The cite-scanner reads `\cite{key}` / `\citet{key}` / `\citep{key}` from those files. Currently no paper has draft prose yet — only dossier files at the paper-dir root.
...
This repo starts the citation database empty (`refs/entries/` is `.gitkeep`-only). Entries land in either of two ways as drafts begin:

```bash
# Bulk-import from an existing .bib (e.g., a hand-curated starting set):
bin/refs import some-starter.bib

# Or interactive add when a citation appears in draft prose:
bin/refs add nagel-1974-bat
```

`bin/refs lint` runs the deny-list against every entry plus every key cited in any `<paper-dir>/src/*.md` — **the linter is the anonymization gate before submission**. Run it before each Synthese/Inquiry submission; surfaces self-citations, the Zenodo DOI, framework names, and cohort ELI names that would deanonymize.
