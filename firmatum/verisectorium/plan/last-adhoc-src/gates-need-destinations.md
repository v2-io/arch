...
- Adjacent: [[influx-queues]] (a gate's non-happy outcome must split into at least two, and be stored), [[orientation-gate]] (the estate's one gate with a genuine destination — write access — and a graduated incentive design), [[layer-speeds]] (the membrane a gate sets permeability on), [[evidence-ledgers]] (independence, from the evidence side).
- **Cross-reference owed, not a conflict:** (B)'s act-not-disposition thesis and its six-checks specimen are the same finding [[role-activation]] carries from the onboarding side, drafted in parallel from the same source. Two chapters legitimately need it — how a gate is *written* versus how a role is *activated* — but they should cite each other rather than each look like the origin.
