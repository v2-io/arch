...
The estate has the rung built in one place and not generalized: a terminology store where every naming decision is an append-only event file under the term's own slug — 160 events across 149 term directories, each naming the decider, the action, the timestamp and the reason, addressable by term ([[terminology-store-anatomy]]). Its README states the argument plainly: mutable status fields lose the history, and lose it *silently*, when overwritten. The contrast in the same estate is a 1,405-line changelog carrying the same class of information greppable but not addressable — you can find it if you already suspect it exists, which is the failure mode of every unindexed history.
...
**Two smaller disciplines belong here because nothing else claims them.** A refuted rationale is *deleted*, not archived in place — one live instance removed a trade study entangled with an unsound framing and replaced it with an explicit anti-reconstruction instruction (*"do not inherit a prior conclusion or reconstruct the argument from memory; weigh it fresh when it is next worked"*) plus a neutral statement of current state, so the deletion does not leave a vacuum that invites reconstruction. Worth noting against itself: that deletion did not propagate to two older stores in the same estate, which still carry the trade study verbatim. And **records that predate their own governance need an honest re-grading path** — classified at the grade the current process would have assigned, *marked as retroactive*, never silently upgraded.
...
Inherited at one remove and marked as such: the deleted-rationale instance and the counts-are-pointers norm are read from a live README by an earlier pass of this project and re-stated here without my own re-reading of that file; the re-grading obligation is Joseph's, recorded in a design dialog and asserted by no segment anywhere, including this one — it is carried above as an obligation *named*, not as a practice observed.
