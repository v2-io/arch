...
(A) and (C) are as strong as the definitions of the two speech-act forms — no empirical content. (B) is **heuristic**: motivated by the structural contrast plus lived specimens — the sharpest being a rename correction that sat complete and correct in vivarium's append-only decisions ledger for eleven days, invisible, until a segment's formal expression had to state present truth and the contradiction surfaced (reported by Fable from live vivarium consolidation work, 2026-07-24; relayed, not witnessed by this corpus's authors). This formulation already exists at the same tier and scope discipline as `form-present-truth-collision` in the live udon theory corpus (`udon/v2/theory/src/`) — this segment is its restatement in verisectorium's own voice, not an independent second arrival; the agreement is shared authorship, not corroboration. Max honest attainment without outside evidence: robust-qualitative, and only if a differently-authored corpus shows the same asymmetry doing detection work.

## Working Notes

- Unintegrated influx behind the specimen account (do not cite as warrant): the cross-corpus generalization record and the udon-theory segment copies under `plan/INFLUX/` — the live udon theory corpus is the citable prior landing; the influx copies exist only until integrated or archived.
...
- **2026-08-05 dispatch (fresh-eyes pass).** The udon-theory copies named above have been archived — cite the live corpus `~/src/MOVED/udon/v2/theory/src/`. A **second specimen** for the asymmetry, from the failure side, is registered as R20 in `plan/INFLUX-DISPATCH-2026-08-05.md`: append-only supersession could not signal deadness in autopax (a body banner was needed because frontmatter supersession did not stop readers) — differently sited from the vivarium rename case, and the register also carries the reading that `empirica:`, `OUTLINE-accepted.md` and `lint-outline`'s stage check are one asymmetry seen from several sides.
