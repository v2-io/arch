...
- **Resurrectability** is not an edge kind and not a new field at all — it's already expressible as the axis combination `activity=paused` + `lifecycle=reference` (Joseph 2026-09-01): dormant by choice, still influence-bearing, could be its own thing again. The taxonomy carries it for free. (Refines the earlier in-session carve, where it rode the edge type; the inbox.md entry stands as the original record.)
