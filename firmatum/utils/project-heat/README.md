...
- `projects heatmap` — one jet (navy→cyan→yellow→red) row per `active*` project, last 120 days (newest at the left), names right-aligned and elided at 29. Color is log1p-normalized across the whole grid so the long tail stays distinct; the scale bar is the legend.
