...
- CLI conventions (arch/principles pass, 2026-09-01): exit vocabulary 0/1/2/130 (SIGINT clean, broken pipe quiet); `--version` = name + semver + sha while untagged; `--format=json` on the listing (census records: facts, ISO times) and on `heatmap` (per-project day counts + scale); refusals name the class and offer a next action; `projects feedback <text…>` appends provenanced friction reports to `inbox.md` here; help carries commands/examples and teaches that the tool writes `.practica` into project roots.
