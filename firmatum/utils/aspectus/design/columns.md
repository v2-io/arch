...
- **Alignment.** Padded columns (readable, diff-noisy) vs unpadded field-per-fact (diff-clean, ragged). Possibly split by reader: human look pads, machine look does not — but that splits "the same look," which text/JSON have so far refused to do. Not decided.
