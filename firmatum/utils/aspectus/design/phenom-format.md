...
## Finding — SIGNA in a column is not SIGNA in a stream (2026-08-23, Joseph + coordinator, same evening it shipped)

Joseph: *"when it's all lined up in a column it's not the same as when it's a single delta getting reported over and over again."* The coordinator's firsthand read, independently: the eye reads **run length** as strongly as glyph identity, and under the mixed radix length is non-monotonic — `╍╍╍╍╍╍╍╍╍` (9 min) is wider than `══━━` (2 h 20), so a longer run can be *younger*; misread twice in one look. The notation was designed for a stream (one delta at a time; a growing run means "longer since"; nothing competes). In a column the reader compares across rows, so length becomes the channel, and the radix breaks it.

*Why open (carve-out with its reason):* the default was decided hours earlier and the glyph law is the Zoetica primary's; the defect is the **column form**, not the notation. Candidates, none chosen:
1. **Fixed width per rank** — pad each rank to its max count (`═══━━━━━` always eight cells for h+10min) so length is monotonic *within* a rank and rank glyphs carry the rest; costs width (max counts sum to ~13 for the two-rank form).
2. **One rank only**, the most significant, repeated by count — `══` = 2 h, `○○○` = 3 d — monotonic within rank, two-cells-to-six; loses the second rank's precision (which the `relative` format keeps one key away).
3. **A graded density cell for age**, like heat's block — `  ░ ░░ … ██` over a log scale of elapsed time — perfectly monotonic, two cells, pairs visually with heat; loses the unit register SIGNA carries (you can't read "weeks" off it).
4. Keep SIGNA for the **stream** uses it was born for — the header's "since last look" delta, file-as-PATH, last-look marks — and use `relative` (`13.6d ago`) in the column.

The honest test is the one already specified for glyph packs: a fresh-agent cold read of a column rendered each way, asking it to order rows by age. Joseph's call which to try first; (3) is the most aspectus-native, (4) is the cheapest and keeps SIGNA where it's strongest.
