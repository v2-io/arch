...
| has (contents of the place) | INFO | ON | — | — | NO | facet* / first-line | — | — | — | Y | — | The parent line's gathering spot, renamed `kind:`→`has:` (Joseph, 2026-08-14): the map detects *contents*, not identity — `[has: git, rust, …]` claims exactly what a glob can know (`.pytest_cache [has: git]` is true; `[kind: git]` was a lie that poisoned the facet's credibility, per the cross-arch hallway test). **Identity claims are the specialized facets** (`[git: remote (private) br stat]` — earned by a plugin that verified the real thing). Furniture *feeds* this fact. Filter is `--has`, not a cut of children. |
...
| initial-sha / latest-sha | Y | OFF | — | Y | COMPOSE | short* / h~n / full | — | — | — | NO | — | One row now: both are Heat-family obtains, compose-only, omit outside git. **Columns shipped 2026-08-14** (`columns.initial-sha` / `.latest-sha`, files only); sort keys still unbuilt/refused. |
