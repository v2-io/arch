...
- **kind-word** (quiet): speaks when a node's **countable class** (counts lines: `text`, `data`, `log`, `exe/script`, `image/svg`, `empty` — vs. does not: `image`, `doc`, `archive`, `media`, `font`, `object`, `model`, `exe/binary`, `binary`) differs from its level's plurality, and then says the *major* (`doc`, `image`, …). *Decided 2026-08-22 at step 4's landing: comparing raw majors made every `Cargo.toml`/`package.json` speak `data` in text-majority dirs — a manifest among prose is not a surprise; a PDF among `.md` is.*
...
- ~~Whether `exe` is a major or a *trait*~~ **Landed as the leaning (2026-08-22, step 4):** `exe` is a **trait** wherever a suffix already names the language (`script.py` + shebang + `+x` → `text/python`, trait `python`); an *extensionless* shebang file is `exe/script`; native binaries from magic are `exe/binary` (a type, not a `+x` mark). The ladder table's step 5a ("shebang decides `exe/script`") is therefore scoped: shebang *decides* only when no suffix names the language; otherwise it supplies the trait — so a crate of `*.py` never splits into `text×N · exe×M`. Also landed: `empty` as its own major (line-count still shows `0`); the kind-word speaks the *major* (`image` among `.md`, not `binary`); `format.census = suffix* | minor | major`. Not changed: census buckets stay suffix-keyed by default.
