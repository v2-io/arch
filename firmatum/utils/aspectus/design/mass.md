...
## Open — the headline number's stability (2026-08-22 cold read, Grok: "directory line-totals are not a fact I can carry to the next look")

*Why open:* the deep line total on an unexpanded dir is computed inside a per-look share of the read budget, so the same directory reads `≥ 1.0M` / `~ 2.4M` / `~ 679.1K` / `~ 1.1M` under different `--lines`/`--depth` in one minute (`audit/hallway-2026-08-22.md` #9). The `~` mark confesses it, and the reader still carries the first number. Candidates: (a) a budget-independent estimate — size-based always, marked `~`, exact only when the whole subtree was actually read; (b) exact counts from the cache row once it exists; (c) keep as is and teach. The trust cost is on the tool's single most-quoted number; Joseph's call.
