...
**Step 5 landed (2026-08-22) — two silences in the design, decided at landing (coordinator; Joseph corrects on contact):** (a) **Directory cells:** porcelain reports an untracked directory as `?? dir/` and says nothing about the children; the cell shows **`⁇` on the directory** (a newly-added directory that looked clean would be a silent lie — the one thing this tool must not do); tracked-and-clean dirs stay blank; a gitignored dir shows `⊘`. (b) **Look-wide presence:** once the look contains any live work tree, every *row* pays the cell (blank when clean) so the tree prefix never dances — lattice-2's "usually known fixed width" read look-wide; **the stamp, root-facts, and root-path header lines do not pay it** (a copied root path must not carry a leading space), the headings line does (it sits over the rows). A non-git sibling of a nested repo pays the blank cell — accepted.

- **Aliveness (heat) — a two-cell density block, far-left, before git-status** (Joseph, 2026-08-23: *"change heat to { ░▒▓█} double column ('  ', ' ░', '░░', …) far left before git status even"*). Nine grades from the four density glyphs filling left to right: `  ` · ` ░` · `░░` · `░▒` · `▒▒` · `▒▓` · `▓▓` · `▓█` · `██`; blank = no heat (outside git, or score 0 — absent never faked, and 0 reads as blank by construction). Score → grade: the heat score is ~0–2 in-repo (git-heat's normalization); quantize linearly over [0, 2] into grades 1–8 with any positive score at least ` ░`, capped at `██` (the mapping is the implementer's first cut; Joseph corrects on contact). The numeric score leaves the far-right cluster; **the far-right keeps the age** under an `age` heading (the relative mtime that was the cluster's right half); `format.heat = density* | score` keeps the number reachable. Both cold readers read density bars as magnitude without a legend (`design/glyphs.md`), which is why this pack can ship without the cold-read test the others wait on.
- **Far-left block padding: two spaces between the block and the tree prefix, overridable** (Joseph, 2026-08-23 — the cramped `M├──` both readers tripped on). Config: `layout.far-left-gap = 2` (spelling provisional), rides the caller stack. The gap is paid only when the block exists (same look-wide presence rule).
