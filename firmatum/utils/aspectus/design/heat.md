...
## Open (2026-08-14 close audit — Joseph's)

- **Heat is repo-relative and the look doesn't yet bound it structurally.** The audit's specimen: `--sort heat ~/src` ranks `ruby-community/` 1.08 (7.1 months dormant) above `arch/` 0.33 (today's work) — the magnitude channel answers "which repo's recent commits were concentrated" while reading as "where is work alive." Wave E shipped the honest *claim scope* (help + model note above); the structural fix is design-grade and unshipped. Options on the table, none chosen: normalize decay by wall-clock instead of commit-count when repos mix; suppress or refuse `--sort heat` above repo boundaries; scope the score per-repo and sort repos by a different aliveness fact. Cheap-mark proposal, for ratification rather than shipped: when a heat *sort's* comparison set spans more than one repo, the look carries one header-adjacent mark (e.g. `[heat: per-repo scale]`, spelling open) — the case that inverts the answer is exactly the case that gets the confession.
- **Noise basenames (revised).** Aspectus now ships `NOISE_BASENAMES = ["SOURCE_REV"]` — a deliberate departure from git-heat's `{Cargo.toml, SOURCE_REV}`: that pair was tuned for repos with mechanical per-commit version stamps (SOURCE_REV's presence is the tell), where Cargo.toml churned automatically; in a general glance a Cargo.toml edit is dependency *intent*. The audit also caught the silencing self-defeating (heat absent → quiet's raw mtime spoke → the "silenced" file was the loudest row); fixed structurally — a git-known unscored line now carries its age in the cluster (score absent, never faked), so noise/no-score files stay in the one time register. Whether `Cargo.lock` (mechanical churn) *joins* noise, and the noise map's merge into the furniture/config map, remain Joseph's.
