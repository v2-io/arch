...
fn root_facts_get_their_own_line_path_stays_bare() {
    ...
    // mtime's default form is SIGNA (2026-08-23); the claim under test is
    // only that the facts line is its own line and the path line stays bare.
    assert!(
        (facts.contains('·') || facts.contains('╶') || facts.contains('╌') || facts.contains('╍'))
            && !facts.contains('/'),
