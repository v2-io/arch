...
(a) LLM summarizer (the industry default); (b) structural fold — HIPIF's [g_k, o_k^end] rule applied by harness, no training; (c) **salience-gated verbatim retention** — keep segments by measured worst-case-over-observed T1/T2 score with a per-head prior floor (DefensiveKV's aggregation, transplanted to consolidation), summarize only the residue; (d) full context (ceiling); (e) recency window (floor).
