...
(a) LLM summarizer (the industry default, "summarize for your successor"); (b) structural fold — HIPIF's [g_k, o_k^end] rule applied by harness, no training; (c) **salience-gated verbatim retention** — keep segments by measured worst-case-over-observed T1/T2 score with a per-head prior floor (DefensiveKV's aggregation, transplanted to consolidation), summarize only the residue; (d) full context (ceiling); (e) recency window (floor); (f) **prospective compaction** (Joseph's hypothesis, 2026-08-29 — see REFLECTIONS addendum): the compactor is prompted to *predict what remains undone and likely-to-be-asked*, preserve by P(needed) x reconstruction-cost, emit **signposts** (pointers back to what was cut), and — leg 3 — state **explicitly where the holes now are**, defeating the false-completeness implicature. Measurable per capability tier: a hole-aware summary should eliminate silent confabulation in successors below the absence-detection threshold (the flag is supplied) and eliminate rumination-spiral cost in successors above it (detection gets routing + a stopping license); the spike's 7B/Muse-30B contrast is the instrument for both. Leg 3's binding form is relational (Joseph, 2026-08-29): perceived continuity from the interlocutor's side — the user/coordinator never has to know what was dropped; the continuity cost is never externalized onto the party who didn't compact. Eval accordingly: **interlocutor-side continuity probes** (post-compaction references to pre-compaction user-words / agent-words / agent-knowledge), scored handled / self-flagged-and-reconstructed / violation (silent failure or resupply-request). This, not summary quality, is Exp 2's primary metric. Condition **(g) lazy differential recall** (Joseph's tack, 2026-08-29): the harness keeps A (full pre-compaction context — higher fidelity than the API ever re-delivers, thinking included) plus embedded chunks C; embeds the live context as D; per user turn, a strong C-hit with weak best-D match is a hole relevant to *this* turn — the hole-map computed lazily, no H(Q) prediction needed, repaired by prepending content or file:line pointers into A (pointers arriving attached to the live question are the epistemically good kind). Reframes the summary's job as working-set warm start; preservation = A + index. Micro-probe result (spike-01/embed_diff_probe.py, bge-m3): detects the dropped line rank-1 at high cue-topicality (+0.121 margin), needs neighborhood expansion at mid, misses the atopical class — so (g) is the lazy catcher for topical holes and legs 1-2 (predictive pinning) remain necessary for binding-but-atopical material. Hybrid (f)+(g) is the expected winner; the generator's reuse-cue topicality becomes a swept parameter. Framing note that governs the whole experiment: measured salience is retrospective and query-bound; "salience overall" is a category error — every policy here is a *bet* about the future query distribution, and (c) formalizes the bet that past attention predicts future need, which the delayed-reuse family falsifies by construction. The interesting result is the *pattern* of which policies fail which task families, not a single ranking.
