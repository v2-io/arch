...
- **The lazy-differential insight**: the harness holds the full pre-compaction context A at *higher* fidelity than the API ever re-delivers — with a provider-conditional caveat (measured in real grok-build logs, 2026-08-29): Anthropic-style APIs stream actual thinking text, so A includes it; xAI's Responses API delivers only a few-sentence summary plus an `encrypted_content` blob undecryptable locally, so for grok-hosted reasoning models A's thinking layer is summaries-only and **no harness change can recover what the API encrypts**. Preservation is therefore **A + index**, and the summary's job contracts to *working-set warm start*. Hole-detection can then be computed lazily per turn (embed A's chunks and the live context; a strong A-hit with weak live-context match is a hole relevant to *this* turn) — sidestepping the H(Q) prediction problem for the recall function by waiting for Q instead of predicting it.
