...
Pedagogy teaches the system in vivo, and feeds forward: what a council repeatedly does on advice can crystallize — in-band — into its standing rules, its custom (2:25, which the model tracks as a rule class), and eventually special rules of order. Hints never decide; adoption converts teaching into law.
...
1. *Probability never gates; rarity never buries.* RONR deliberately protects rights whose exercise is rare — point of order, appeal, division, the lone objection to unanimous consent are the least-frequent and most load-bearing actions in the system, and frequency-ranking would progressively hide exactly the minority-protective machinery. Rights-critical affordances (roughly: everything a single member can demand) are pinned — exempt from ranking, always visible when live.
